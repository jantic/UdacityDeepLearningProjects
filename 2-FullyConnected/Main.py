...
def multilayerNetwork(_X, _weights, _biases):
  ...
  hidden2 = tf.nn.relu(tf.matmul(hidden1, _weights['h2']) + _biases['h2'])
  outputLayer = tf.matmul(hidden2, _weights['out']) + _biases['out']
  return outputLayer
...
with graph.as_default():
  ...
  weights = {
    ...
    'h2':tf.Variable(tf.truncated_normal([num_hidden_params, num_hidden_params])),
    'out':tf.Variable(tf.truncated_normal([num_hidden_params, num_labels]))
  }

  biases = {
     'h1':tf.Variable(tf.zeros([num_hidden_params])),
     'h2':tf.Variable(tf.zeros([num_hidden_params])),
